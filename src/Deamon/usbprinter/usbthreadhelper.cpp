...
/*判断打印机是否已经添加过*/
static bool generateUSBDeviceInfo(const map<string, string> &infoMap, TDeviceInfo &deviceInfo)
{
    ...
    qInfo() << "deviceInfo.strClass: " << deviceInfo.strClass;

    return false;
}

USBThreadHelper::USBThreadHelper(QObject *parent)
    : QObject(parent)
...
bool USBThreadHelper::addArrivedUSBPrinter()
{
    ...
    if (pDriverSearcher) {
        ...
        foreach (auto driver, drivers) {
            if (driver.contains(SD_KEY_excat) && driver[SD_KEY_excat].toBool()) {
                ...
                connect(task, &AddPrinterTask::signalStatus, this, &USBThreadHelper::addingJobFinished);
                task->doWork();
...
void USBThreadHelper::notificationActionInvoked(uint id, const QString &msg)
{
    Q_UNUSED(msg)
...
void USBThreadHelper::nextConfiguration()
{
...
void USBThreadHelper::addPrinter(unsigned int vid, unsigned int pid)
{
    getUsbDevice(vid,pid);
    if (m_usbDeviceList.count() > 0 && (m_currentUSBDevice == nullptr)) {
        emit newUSBDeviceArrived();
    }
}

void USBThreadHelper::addingJobFinished(int status)
{
    QString strReason = "";
    if (status == TStat_Suc) {
...
void USBThreadHelper::getDriver()
{
    ...
    connect(pDriverSearcher, &DriverSearcher::signalDone, this, &USBThreadHelper::addArrivedUSBPrinter);
    pDriverSearcher->startSearch();

}

void USBThreadHelper::processArrivedUSBDevice()
{
    ...
    isAdded = generateUSBDeviceInfo(infoMap, m_deviceInfo);
    if (!isAdded) {
